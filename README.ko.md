...
| 네트워크 | TCP/IP, UDP, DNS, HTTP, QUIC, TLS, Proxy, Load Balancing, CDN, Cache |
...
- [15. 이해도 확인 문제](quizzes/networking/15-cdn-http-cache.md)
...
WebSocket → Server-Sent Events → gRPC → REST vs RPC → API Gateway → Rate Limiting → Retry / Timeout / Circuit Breaker 순으로 진행합니다.
